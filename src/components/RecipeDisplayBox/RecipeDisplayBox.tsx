...
export default function RecipeDisplayBox(props: { idReceita: string; imagem: string; nome: string; categoria: string;  tempo: string; ingredientes: []}){
    ...
    const { cor: corDaCategoria } = categoriaMap[categoria] || { cor: "bg-gray-200", texto: "- - -" };
      
    return (
        // ARRUMAR O LINK DEPOIS
        <Link href={`/receita/${idReceita}`} className="group flex flex-row md:max-w-96 items-center gap-4 border-2 border-black rounded-xl p-4 max-w-full bg-white hover:bg-gray-300">
          
          {/* Imagem */}
          <div className="w-24 h-24 flex-shrink-0 overflow-hidden rounded-lg bg-amber-300">
            <Image
              src={imagem}
              alt={`Imagem de ${nome}`}
              width={96}
          ...
          <div className="flex flex-col justify-between flex-1 gap-2 bg-white p-2 rounded-lg text-left group-hover:bg-emerald-50">
            <div className="flex flex-col gap-4">
                ...
                <div className="flex gap-4 justify-start items-center">
                    <h3 className="text-sm text-gray-800"><i className="fa-solid fa-clock" /> {tempo}</h3>
                    <h3 className="text-sm text-gray-800"><i className="fa-solid fa-spoon"/> {ingredientes.length} ingredientes</h3>
                </div>
            </div>
          </div>
        </Link>
